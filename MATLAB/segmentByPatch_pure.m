function [tmaps,ntmaps,tmaps2,ntmaps2,cm1,cm2] = segmentByPatch_pure(pd,gt)
...
gtp=imerode(gt,strel('disk',256,8));
gtn=imerode(~gt,strel('disk',256,8));
gtg(gtp)=1;
gtg(gtn)=0;
gtg(~gtp&~gtn)=0.5;
gtg=reshape(gtg,size(gt));

cm1=getCM_pure(tmap,ntmap,gtg);
cm2=getCM_pure(tmap2,ntmap2,gtg);
